...
label_file = os.path.join(dir, 'class_comparison_tabel_zh_imagenet1k')
print(label_file)
model_list = ['vit_base_patch16_224']


def get_labels(task, en=False):
    labels = []
    if task == 'common':
...
def vit_inference(img, model, task='common'):
    ...
    class_names = get_labels(task)  # 替换为实际的类别名称
